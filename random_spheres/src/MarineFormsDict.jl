...
function generate_geometry(animal::PorousSphere)
    gmsh.model.occ.addSphere(animal.x, animal.y, animal.z, animal.r)     # Add a porous sphere gmsh object
end

function generate_geometry(animal::RigidSphere)
    gmsh.model.occ.addSphere(animal.x, animal.y, animal.z, animal.r)     # Add a rigid sphere gmsh object 
end
...
"This function checks if exists overlapping between two marine forms.
Since at this time we only have Clamps and Spheres, we can use the exterior radius
to check the overlapping and the entry of the function can be an abstract animal. At the 
...
#     return dist < (form1.re + form2.re + max(form1.tol, form2.tol) )  # Condition to have a overlapping
# end

function check_collisions(form1::Sphere, form2::Sphere)
    dist = sqrt((form1.x - form2.x)^2 + (form1.y - form2.y)^2 + (form1.z - form2.z)^2)
    
    return dist < (form1.r + form2.r + max(form1.tol, form2.tol))  # Condition to have a overlapping
end

# + max(form1.tol, form2.tol)
"""
This function checks if exists overlapping between the animal created and the previously created animals.
