...
object = gmsh.model.occ.importShapes("./geometry/cube.brep")
# # Since the geometry is imported in milimeters from FreeCAD is recommendable scale it to meters
gmsh.model.occ.synchronize()
...
# Translate the geometry to the center of the domain
gmsh.model.occ.translate([(3, object[1][2])], 0, (H-ymax)/2, 0)
...
for surface in surfaces
    ...
    if xmin < com[1] < xmax && ymin < com[2] < ymax && zmin < com[3] < zmax
        push!(surface_little_cube_marker, surface[2])
...
cube_little_tag = gmsh.model.addPhysicalGroup(2, surface_little_cube_marker)
...
gmsh.model.setPhysicalName(2, cube_little_tag, "Cube")
...
gmsh.write("./data/coarse.msh")

# Finalize Gmsh session
gmsh.finalize()

# Convert the mesh to the Gridap format
model = GmshDiscreteModel("./data/coarse.msh")

# Write the mesh to a vtk file
writevtk(model,"./results/coarse")

names_meshes = ["data/coarse", "data/medium", "data/fine"]
result_meshes = ["results/medium", "results/fine"]

for i in 1:(length(names_meshes)-1)
    gmsh.initialize()
    gmsh.open(names_meshes[i]*".msh")

    gmsh.model.mesh.refine()

    gmsh.write(names_meshes[i+1]*".msh")

    gmsh.finalize()

    model2 = GmshDiscreteModel(names_meshes[i+1]*".msh")
    writevtk(model2, result_meshes[i])
end
